...
const Todos = (0, import_index_ff5e144f.c)(($$result, $$props, $$bindings, slots) => {
  let { todos } = $$props;
...
<div class="${"todos svelte-1dauu4f"}"><h1>Todos</h1>
	...
	${(0, import_index_ff5e144f.l)(todos, (todo) => {
    return `<div class="${["todo svelte-1dauu4f", todo.done ? "done" : ""].join(" ").trim()}"><form action="${"/todos/" + (0, import_index_ff5e144f.e)(todo.uid) + ".json?_method=patch"}" method="${"post"}"><input type="${"hidden"}" name="${"done"}"${(0, import_index_ff5e144f.b)("value", todo.done ? "" : "true", 0)} class="${"svelte-1dauu4f"}">
				<button class="${"toggle svelte-1dauu4f"}" aria-label="${"Mark todo as " + (0, import_index_ff5e144f.e)(todo.done ? "not done" : "done")}"></button></form>

			<form class="${"text svelte-1dauu4f"}" action="${"/todos/" + (0, import_index_ff5e144f.e)(todo.uid) + ".json?_method=patch"}" method="${"post"}"><input aria-label="${"Edit todo"}" type="${"text"}" name="${"text"}"${(0, import_index_ff5e144f.b)("value", todo.text, 0)} class="${"svelte-1dauu4f"}">
				<button class="${"save svelte-1dauu4f"}" aria-label="${"Save todo"}"></button></form>

			<form action="${"/todos/" + (0, import_index_ff5e144f.e)(todo.uid) + ".json?_method=delete"}" method="${"post"}"><button class="${"delete svelte-1dauu4f"}" aria-label="${"Delete todo"}" ${todo.pending_delete ? "disabled" : ""}></button></form>
		</div>`;
  })}
</div>`;
});
